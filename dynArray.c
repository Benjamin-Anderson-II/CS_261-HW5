...
void addHeap(DynArr *heap, TYPE node)
{
	assert(heap);
  	...
  	addDynArr(heap, node);
  	_adjustHeap(heap, heap->size-1, heap->size-1);
}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  	...
  	if(parentIndex>=0 && _smallerIndexHeap(heap, parentIndex, pos)==pos)
  	{
  		swapDynArr(heap, parentIndex, pos);
  		_adjustHeap(heap, max, parentIndex);

  	} else if(leftChild<=max && _smallerIndexHeap(heap, pos, leftChild)==leftChild)
  	{
  		swapDynArr(heap, pos, leftChild);
  		_adjustHeap(heap, max, leftChild);

  	} else if(rightChild<=max && _smallerIndexHeap(heap, pos, rightChild)==rightChild)
  	{
